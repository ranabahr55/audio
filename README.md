...
```
  ...
  │ → Zenoh put (express, RT)   │ audio/.. │           ↘ live playback    │
...
* **Subscriber** (`audio_sub/`) is Python: decodes Opus by calling the system
  ...
  using the per-packet sequence number, streams the result to **FLAC**
  via `soundfile`, and — unless disabled — **plays it live to your speakers**
  as it arrives via `sounddevice`. A dedicated playback thread keeps the
  speaker clock from blocking decode/disk I/O; if playback falls behind,
  frames are dropped to stay real-time while the FLAC file stays lossless.
...
**Terminal 2 — subscriber** (receives, decodes, plays live + saves):
...
You should hear the mic live as packets arrive, while the audio is also
written to FLAC. Set `playback = 0` in `config.cfg` to save only (no speakers).
The status line shows `play_dropped=…` — frames discarded when the speaker
fell behind (the saved file is unaffected).
...
`low_delay`, plus the subscriber's `output` path and `playback` toggle
(`playback = 1` plays live to speakers, the default; `0` saves only).
...
to install. FLAC encoding uses `libsndfile` through `soundfile`. Live playback
uses `sounddevice` (PortAudio); if it is not installed (`pip install
sounddevice`) the subscriber prints a warning and falls back to saving only.
